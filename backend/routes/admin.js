...
router.get("/", function (req, res, next) {
  ...
  adminHelper.getAllShops().then((shops) => {
    res.json( { admin: true, shops, administator });
  });
});
...
router.get("/all-shops", verifySignedIn, function (req, res) {
  let administator = req.session.admin;
  adminHelper.getAllShops().then((shops) => {
    res.render("admin/all-shops", { admin: true, shops, administator });
  });
});
...
router.post("/add-shop", function (req, res) {
  adminHelper.addShop(req.body, (id) => {
    let image = req.files.Image;
    image.mv("./public/images/shopimages/" + id + ".png", (err, done) => {
      if (!err) {
...
router.get("/edit-shop/:id", verifySignedIn, async function (req, res) {
  let administator = req.session.admin;
  let shopId = req.params.id;
  let shop = await adminHelper.getShopDetails(shopId);
  console.log(shop);
  res.render("admin/edit-shop", { admin: true,shop, administator });
});

router.post("/edit-shop/:id", verifySignedIn, function (req, res) {
  let shopId = req.params.id;
  adminHelper.updateShop(shopId, req.body).then(() => {
    if (req.files) {
      let image = req.files.Image;
      if (image) {
        image.mv("./public/images/shop-images/" + shopId + ".png");
      }
    }
    res.redirect("/admin/all-shops");
  });
});
...
router.post("/delete-shop/:id",  function (req, res) {
  let shopId = req.params.id;
  console.log('proid',shopId);
  
  adminHelper.deleteProduct(shopId).then((response) => {
    // fs.unlinkSync("./public/images/product-images/" + productId + ".png");
    res.json({message:'delete success'});
  });
});
