...
module.exports = {
  ...
  getSignedUserShops:(email)=>{
    console.log('mail is',email);
    return new Promise(async(resolve,reject)=>{
      let items= await db.get().collection(collections.SHOPS_COLLECTION).find({CreatedBy:email}).toArray()
      resolve(items);
  ...
  editshop:async(userid)=>{
    const shopData=await db.get().collection(collections.SHOPS_COLLECTION).findOne({_id:objectId(userid)})
      return shopData
